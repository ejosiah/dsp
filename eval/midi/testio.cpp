...
void main_test_input(unsigned int somethingStupid) {
    ...
    while (i < num) {
        status = Pm_Poll(midi);
        if (status == TRUE) {
            length = static_cast<PmError>(Pm_Read(midi, buffer, 1));
            if (length > 0) {
...
#define chord_size 5
    ...
    Pm_OpenOutput(&midi,
                  ...
                  sysdepinfo,
                  OUTPUT_BUFFER_SIZE,
                  (latency == 0 ? nullptr : TIME_PROC),
                  (latency == 0 ? nullptr : TIME_INFO),
                  latency);
...
#define PROGRAM 0
    ...
    if (isochronous_test) { // play 4 notes per sec for 20s
        ...
        PmError err = static_cast<PmError>(0);
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < Pm_CountDevices(); i++) {
        ...
        if (((test_input  | test_both) & info->input) |
            ...
            if (info->input) {
                deflt = const_cast<char*>(i == default_in ? "default " : "");
                printf(" (%sinput)", deflt);
            }
            if (info->output) {
                deflt = const_cast<char*>(i == default_out ? "default " : "");
